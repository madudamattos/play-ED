...
void retiraMusica(ListaDeMusica *lista, Musica *musica) {
  ...
  // Procura a célula da música na lista
  while (p != NULL && !ehIgualPonteiroMusica(p->musica, musica)) {
    anterior = p;
    p = p->prox;
  }

  // Se a música não estiver na lista, saia da função
  if (p == NULL)
    return;

  // Se a música estiver no início da lista
  if (anterior == NULL) {
    lista->prim = p->prox;
  } else {
  ...
  // Se a música estiver no final da lista
  if (p->prox == NULL) {
    lista->ult = anterior;
  }

  // Libera apenas a célula da música, não a memória da música
  free(p);
}

char *retornaNomeMusica(Musica *musica) { return musica->nome; }

char *retornaArtistaMusica(Musica *musica) { return musica->artista; }

// Retorna 1 se encontrou, retorna 0 se nao encontrou
...
// verifica se os dois ponteiros apontam para o mesmo lugar
...
// Retorna um ponteiro pra musica se encontrar, se não encontrar retorna NULL
